...
class Rule(object):
    ...
    def ls_log(self):
        """
        Read all ls_log.xm information
        :return:
        """
        ls_log_info = {}
        ls_logs = self._read_xml('ls_log.xml')
        if ls_logs is None:
            logger.critical('ls_log read failed!!!')
            return None
        for ls_log in ls_logs:
            ls_log_id = int(ls_log.get('name'))
            ls_log_msg = ls_log.get('msg')
            ls_log_value = ls_log.text
            ls_log_info[ls_log_id] = (ls_log_msg, ls_log_value)
        return ls_log_info
