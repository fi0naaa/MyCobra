...
def start(target, formatter, output, special_rules, a_sid=None):
    ...
    s_sid = get_sid(target)     # 获取sid('s' + target前5位经过md5加密 + 6位随机码)
    r = Running(a_sid)
    data = (s_sid, target)
    r.init_list(data=target)
    r.list(data)                # 将sid和target_num写入list文件

    report = '?sid={a_sid}'.format(a_sid=a_sid)
    d = r.status()
    d['report'] = report
    r.status(d)                 # 将报表url后缀参数写入status文件
    logger.info('[REPORT] Report URL: {u}'.format(u=report))

    # parse target mode and output mode   # 解析target和output类型
